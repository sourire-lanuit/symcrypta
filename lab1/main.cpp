...
void print_letters(const std::unordered_map<std::string, long long>& freq, const std::string& filename) {
    std::ofstream out(filename);
    if (!out) return;
    out << std::setw(10) << "symbol" << std::setw(10) << "count" << "\n";
    for (char c : alph_nospace) {
        std::string s(1, c);
        long long count = 0;
        auto it = freq.find(s);
        if (it != freq.end()) count = it->second;
        out << std::setw(10) << c << std::setw(10) << count << "\n";
    }
...
void print_probabilities_to_file(const std::unordered_map<std::string, long long>& freq, long long total, const std::string& filename) {
    std::ofstream out(filename);
    if (!out) return;
    out << std::setw(10) << "symbol" << std::setw(15) << "probability" << std::endl;
    ...
    for (char c : alph_nospace) {
        ...
        out << std::setw(10) << c << std::setw(15) << std::fixed << std::setprecision(6) << prob << std::endl;
    }
...
int main() {
    ...
    print_letters(letters, "letters_frequency_table.txt");
    std::cout << "Table is saved to letters_frequency.txt\n";

    print_probabilities_to_file(letters_wo_space, text_wo_space.size(), "rusprob.txt");
